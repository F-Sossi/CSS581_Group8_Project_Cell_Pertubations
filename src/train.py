...
from model import ComplexNet  # Import your neural network architecture
from model import ComplexAutoencoder  # Import the autoencoder class
from utils import calculate_mrrmse


def train_autoencoder(epochs, lr_rate, batch_size, latent_size, oversample):
...
def train_nn(epochs, lr_rate, batch_size, latent_size, oversample):
    # Check for GPU availability
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    # latent_size = latent_size  # This should be the same value used during autoencoder training
    ...
    # DataLoader for the training data
    train_loader = DataLoader(TensorDataset(X_train_latent, y_train_tensor), batch_size=batch_size, shuffle=True)
